...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    ...
    for(auto elem : populations){
        result[elem]++;
        queue.enqueue({elem, -1.0*populations[elem]/sqrt(2)});
    }
    numSeats -= populations.size();
    while(numSeats > 0){
        DataPoint elem = queue.dequeue();
        int upnum = ++result[elem.name];
        queue.enqueue({elem.name, -1.0 * populations[elem.name] / sqrt(upnum * (upnum + 1))});
        numSeats--;
    }
    return result;
}
