...
void HeapPQueue::enqueue(const DataPoint& data) {
    if(logicalSize + 1 == allocatedSize) expandCapacity();
    elems[++logicalSize] = data;
    int pos = logicalSize;
    while(pos > 1 && elems[pos].weight < elems[pos / 2].weight){
        swap(elems[pos], elems[pos / 2]);
        pos /= 2;
    }
}

void HeapPQueue::expandCapacity(){
    DataPoint *myelems = elems;
    allocatedSize *= 2;
...
DataPoint HeapPQueue::dequeue() {
    ...
    int leftpos = 2;
    int rightpos = 3;
    while((leftpos <= logicalSize && elems[leftpos].weight < elems[pos].weight) ||
           (rightpos <= logicalSize && elems[rightpos].weight < elems[pos].weight)){
        int changgepos = (rightpos <= logicalSize && elems[leftpos].weight > elems[rightpos].weight) ? rightpos : leftpos;
        swap(elems[pos], elems[changgepos]);
        pos = changgepos;
        leftpos = pos * 2 ;
        rightpos = pos * 2 + 1;
    }
