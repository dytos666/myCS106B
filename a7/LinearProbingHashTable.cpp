...
bool LinearProbingHashTable::insert(const string& elem) {
    /* TODO: Delete this comment and the next lines, then implement this function. */
    if(slotsNum == allocNum){
        return false;
    }
    if(!contains(elem)){
        int pos = hashFn(elem);
        for(; elems[pos].type == SlotType::FILLED; pos = (++pos) % allocNum);
...
bool LinearProbingHashTable::contains(const string& elem) const {
    ...
    int pos = hashFn(elem);
    int num = 0;
    while(elems[pos].type != SlotType::EMPTY && num <= allocNum){
        if(elems[pos].type == SlotType::FILLED && elems[pos].value == elem) return true;
        pos = (++pos) % allocNum;
        num++;
    }
    return false;
...
bool LinearProbingHashTable::remove(const string& elem) {
    ...
    if(contains(elem)){
        int pos = hashFn(elem);
        for(; elems[pos].type != SlotType::EMPTY; pos = (++pos) % allocNum){
