...
bool RobinHoodHashTable::insert(const string& elem) {
    ...
    if(contains(elem)){
        ...
        myslot.distance = 0;
        myslot.value = elem;
        while(elems[pos].distance != EMPTY_SLOT){
            if(elems[pos].distance < myslot.distance){
                swap(myslot, elems[pos]);
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    int pos = hashFn(elem);
    int distance = 0;
    while(elems[pos].distance >= distance){
        if(elems[pos].value == elem) return true;
        pos = ++pos % allocNum;
        distance++;
    }
    return false;
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    if(contains(elem)){
        ...
        Slot myslot;
        myslot.value = "";
        myslot.distance = EMPTY_SLOT;
        while(elems[pos].value != elem) pos = ++pos % allocNum;

        int nextpos = (pos+1) % allocNum;
        while(elems[nextpos].distance !=0 && elems[nextpos].distance != EMPTY_SLOT){
            elems[pos] = elems[nextpos];
            elems[pos].distance--;
            pos = nextpos;
            nextpos = ++nextpos % allocNum;
        }
        swap(elems[pos], myslot);
        logicNum--;
        return true;
    }
    return false;
