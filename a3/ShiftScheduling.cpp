...
int sumOfValue(const Set<Shift> myshift){
    int result = 0;
    for(auto elem : myshift){
        result += valueOf(elem);
    }
    return result;
}
Set<Shift> recursion(const Vector<Shift>& shifts, Set<Shift>& myshift, int remainHours, int pos, int lastpos){
    if(pos == shifts.size()){
        return myshift;
    }
    Set<Shift> result;
    Set<Shift> remain = myshift;
    Set<Shift> add = myshift + shifts[pos];
    if( remainHours >= lengthOf(shifts[pos]) && (lastpos == -1 || !overlapsWith(shifts[pos], shifts[lastpos]))){
        Set<Shift> shift1,shift2;
        shift1 = recursion(shifts, remain, remainHours, pos + 1, lastpos);
        shift2 = recursion(shifts, add, remainHours - lengthOf(shifts[pos]), pos + 1, pos);
        return sumOfValue(shift1) >= sumOfValue(shift2) ? shift1 : shift2;
    }else{
        return recursion(shifts, remain, remainHours, pos + 1, lastpos);
    }
}
...
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    ...
    Set<Shift> result;
    Vector<Shift> vectorShifts;
    for(auto elem : shifts){
        vectorShifts.add(elem);
    }
    return recursion(vectorShifts, result, maxHours, 0, -1);
}
