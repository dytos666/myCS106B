...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    ...
    while(result){
        if(result->value == target->value){
            Nucleotide* mydna = result->next, *mytarget = target->next;
            while(mydna && mytarget && mytarget->value == mydna->value){
                mytarget = mytarget->next;
                mydna = mydna->next;
            }
            if(!mytarget) return result;
        }
        result = result->next;
    }
    return nullptr;
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    ...
    Nucleotide* result = dna;
    Nucleotide* deletepos = findFirst(dna, target);
    if(!deletepos) return false;
    if(dna == deletepos){
        Nucleotide* mytarget = target;
        while (mytarget) {
            mytarget = mytarget->next;
            result = result->next;
        }
        ...
        deleteNucleotides(deletepos);

    }else{
        Nucleotide* mytarget = target;
        Nucleotide* endpos = deletepos;
        while (mytarget) {
            mytarget = mytarget->next;
            endpos = endpos->next;
        }
        if(endpos){
            deletepos->prev->next = endpos;
            endpos->prev->next = nullptr;
            endpos->prev = deletepos->prev;

        }else{
            deletepos->prev->next = nullptr;
        }
        deleteNucleotides(deletepos);
    }
    dna = result;
