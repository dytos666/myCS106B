...
EncodingTreeNode* huffmanTreeFor(const string& str) {
    ...
    if(mymap.size() < 2) error("");
    PriorityQueue<EncodingTreeNode*> myqueue;
    for(auto elem : mymap){
        EncodingTreeNode* node = new EncodingTreeNode;
        node->zero = node->one = nullptr;
        node->ch = elem;
        myqueue.enqueue(node, mymap[elem]);
    }
    while(myqueue.size() > 2){
        EncodingTreeNode* node = new EncodingTreeNode;
        double zeroValue = myqueue.peekPriority();
        EncodingTreeNode* zero = myqueue.dequeue();
        double oneValue = myqueue.peekPriority();
        EncodingTreeNode* one = myqueue.dequeue();
        node->zero = zero;
        node->one = one;
        myqueue.enqueue(node, zeroValue + oneValue);
    }
    EncodingTreeNode* node = new EncodingTreeNode;
    EncodingTreeNode* zero = myqueue.dequeue();
    EncodingTreeNode* one = myqueue.dequeue();
 ...
 */
Map<char, string> makeTalbe(EncodingTreeNode* tree, string str){
    ...
    result+= makeTalbe(tree->zero, str + "0");
    result += makeTalbe(tree->one, str + "1");
    return result;
}
Queue<Bit> encodeText(const string& str, EncodingTreeNode* tree) {
    /* TODO: Delete this comment and the next few lines, then implement this. */
    Queue<Bit> result;
    Map<char, string> table = makeTalbe(tree, "");
    for(auto elem : str){
        string mystr = table[elem];
        for(auto digit : mystr){
            Bit bit;
            bit = (digit == '0') ? 0 : 1;
            result.enqueue(bit);
        }
...
void encodeTree(EncodingTreeNode* tree, Queue<Bit>& bits, Queue<char>& leaves) {
    /* TODO: Delete this comment and the next few lines, then implement this. */
    Stack<EncodingTreeNode*> mystack;
    mystack.push(tree);
    while(!mystack.isEmpty()){
        EncodingTreeNode* node = mystack.pop();
        if(isLeaf(node)){
            bits.enqueue(0);
            leaves.enqueue(node->ch);
        }
        else{
            bits.enqueue(1);
            mystack.push(node->one);
            mystack.push(node->zero);
        }
    }
}
