...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    Queue<GridLocation> myqueue;
    Grid<bool> result(terrain.numRows(), terrain.numCols(), false);
    for(GridLocation key : sources){
        if(terrain.inBounds(key) && terrain.get(key) <= height){
            myqueue.enqueue(key);
        }
    }
    while(!myqueue.isEmpty()){
        GridLocation temp = myqueue.dequeue();
        if(!result[temp.row][temp.col] && terrain[temp.row][temp.col] <= height){
            result[temp.row][temp.col] = true;
            for(int i=0; i<4; i++){
                GridLocation nearDirection(temp.row + directions[i], temp.col + directions[i+1]);
                if(terrain.inBounds(nearDirection)){
                    myqueue.enqueue(nearDirection);
                }
            }
        }
    }
