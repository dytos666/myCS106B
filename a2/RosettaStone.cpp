...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
     ...
     * this function.
     */
    Map<string, double> mykGramsIn;
    if(kGramLength <= 0){
        error("kGramLength num is less than zero ");
    }
    for(int i = kGramLength - 1; i < str.length(); i++){
        string key = str.substr(i - kGramLength + 1, kGramLength);
        if(mykGramsIn.containsKey(key)){
            mykGramsIn[key]++;
        }else{
            mykGramsIn.put(key, 1);
        }
    }
    return mykGramsIn;
}
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    if(input.isEmpty()){
        ...
        if(sum == 0){
            error("all value is zero");
        }
    }
    sum = sqrt(sum);
    for(string key : input){
        myNormalize[key] = input[key] / sum;
    }
    return myNormalize;
}
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    for(string key : source){
        myqueue.enqueue(key,source[key]);
    }
    for(int i=0; i < source.size() - numToKeep; i++){
        myqueue.dequeue();
    }
    while(!myqueue.isEmpty()){
        myKGramsIn[myqueue.peek()] = myqueue.peekPriority();
        myqueue.dequeue();
    }
    return myKGramsIn;
}
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    for(Corpus key : corpora){
        double num = cosineSimilarityOf(textProfile, key.profile);
        if(num > max) {
            max = num;
            mycorpora = key;
        }
    }
    return mycorpora.name;
}
