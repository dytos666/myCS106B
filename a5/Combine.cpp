...
Vector<DataPoint> merge(Vector<DataPoint>& vector1, Vector<DataPoint>& vector2){
    int n1 = vector1.size();
    int n2 = vector2.size();
    ...
    int p2 = 0;
    while(p1 < n1 && p2 < n2){
        if(vector1[p1].weight <= vector2[p2].weight){
            result.add(vector1[p1++]);
        }else{
            result.add(vector2[p2++]);
        }
    }
    while(p1 < n1) result.add(vector1[p1++]);
    while(p2 < n2) result.add(vector2[p2++]);
    return result;
}
...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    Vector<Vector<DataPoint>> s1;
    Vector<Vector<DataPoint>> s2;
    for(int i=0; i < sequences.size(); i++){
        if(i < sequences.size() / 2){
            s1.add(sequences[i]);
        }else{
            s2.add(sequences[i]);
        }
    }
    Vector<DataPoint> v1 = combine(s1);
    Vector<DataPoint> v2 = combine(s2);
    return merge(v1, v2);
