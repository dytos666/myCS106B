...
bool recursionOfPlan(const Map<string, Set<string>>& roadNetwork, int restNumCities,
                     Set<string>& supplyLocations, Set<string> unCoverLocations){
    if(unCoverLocations.isEmpty()){
        return true;
    }else if(restNumCities == 0){
        return false;
    }
    string chooseNode = unCoverLocations.first();
    Set<string> notChoose = supplyLocations;
    Set<string> haveChoose = supplyLocations + chooseNode;
    if(recursionOfPlan(roadNetwork, restNumCities - 1, haveChoose, unCoverLocations - roadNetwork[chooseNode] - chooseNode)){
        supplyLocations = haveChoose;
        return true;
    }
    for(auto elem : roadNetwork[chooseNode]){
        notChoose += elem;
        if(recursionOfPlan(roadNetwork, restNumCities - 1, notChoose, unCoverLocations - roadNetwork[elem] - elem)){
            supplyLocations = notChoose;
            return true;
        }
        notChoose -= elem;
    }
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    Set<string> unCoverLocation;
    for(auto elem : roadNetwork){
        unCoverLocation += elem;
    }
    return recursionOfPlan(roadNetwork, numCities, supplyLocations, unCoverLocation);
}
