...
Set<string> permutationsRec(string str, string chosen) {
     ...
     */
        ...
        for (int i = 0; i < str.size(); i++) {
             ...
             * the result.
             */
            Set<string> thisOption = permutationsRec(remaining, chosen + ch);// the problem is the parameter chosen passed by chosen plus ch but not change the
                                                                             // current function parameter chosen value;
            result += thisOption;
        }
