...
bool isMatching(Set<string>& node, string str){
    for(auto elem : node){
        if(elem == str){
            return true;
        }
...
bool isMatching(Map<string, Map<string, int>> node, string str){
    for(auto elem : node){
        if(elem == str){
            return true;
        }
...
bool findPerfectMatching(const Map<string, Set<string>>& possiblelinks, Set<string>& restnode, Set<Pair>& matching){
    if(possiblelinks.isEmpty()){
        return true;
    }
    string nowNode = possiblelinks.firstKey();
    Set<string> nowNodeLinks = possiblelinks[nowNode];
    restnode += nowNode;

    while(!nowNodeLinks.isEmpty()){
        string elem = nowNodeLinks.first();
        nowNodeLinks.remove(elem);
        if(isMatching(restnode, elem)){
            continue;
        }
        Pair mypair(nowNode, elem);
        matching += mypair;
        restnode += elem;
        Map<string, Set<string>> mylinks = possiblelinks;
        mylinks.remove(nowNode);
        mylinks.remove(elem);
        if(findPerfectMatching(mylinks, restnode, matching)){
            return true;
        }
        matching -= mypair;
        restnode -= elem;
...
bool ifExistLink(Map<string, Map<string, int>>&restnode){

    for(auto elem : restnode){
        for(auto matchelem : restnode){
            if(restnode[elem].containsKey(matchelem)){
                return true;
            }
        }
    }
...
int sumOfWeight(const Map<string, Map<string, int>>& possibleLinks, const Set<Pair>& mypair){
    int sum=0;
    for(Pair elem : mypair){
        sum += possibleLinks[elem.first()][elem.second()];
    }
    return sum;
}

Set<Pair> findMaximum(const Map<string, Map<string, int>>& possibleLinks,
                      Map<string, Map<string, int>> &restnode, Set<Pair> mypair){

    if(!ifExistLink(restnode)){
        return mypair;
    }

    string nowNode = restnode.firstKey();
    Map<string,int> nowNodeLinks = possibleLinks[nowNode];
    int max= -100;
    Set<Pair> maxpair;
    Map<string, Map<string, int>> subLinks = restnode;
    Set<Pair> resultPair;
    int sumOf;
    subLinks.remove(nowNode);

    while(!nowNodeLinks.isEmpty()){
        string elem = nowNodeLinks.firstKey();
        nowNodeLinks.remove(elem);
        if(isMatching(possibleLinks - restnode, elem)){
            continue;
        }
        Pair temppair(nowNode, elem);
        Map<string, Map<string, int>> nextLinks = subLinks;

        nextLinks.remove(elem);

        resultPair = findMaximum(possibleLinks, nextLinks, mypair + temppair);
        sumOf =  sumOfWeight(possibleLinks, resultPair);
        if(sumOf > max){
            maxpair = resultPair;
            max = sumOf;
        }
    ...
    resultPair = findMaximum(possibleLinks, subLinks, mypair);
    sumOf =  sumOfWeight(possibleLinks, resultPair);

    return sumOf > max ? resultPair : maxpair;
}
...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    return findPerfectMatching(possibleLinks, restnode, matching);
}

Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    /* TODO: Delete this comment and these remaining lines, then implement this function. */
    Map<string, Map<string, int>> restnode = possibleLinks;
    Set<Pair> mypair;
    return findMaximum(possibleLinks, restnode, mypair);
}
